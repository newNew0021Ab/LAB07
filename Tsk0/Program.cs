...
try
{
    ...
    // average salary task0
    Console.WriteLine("\n=== DEPARTMENT AVERAGES ===");
    
    decimal avgIt = itDepartment.GetAverageSalary();
    decimal avgProd = productionDepartment.GetAverageSalary();

    Console.WriteLine($"IT Dept Avg: {avgIt:F2}"); // F2 ограничивает вывод до 2 знаков после запятой
    Console.WriteLine($"Production Dept Avg: {avgProd:F2}");
