...
public class Department
{
    ...
    // Задание 3: Расчет средней зарплаты 
    public decimal GetAverageSalary()
    {
        decimal sum = 0;
        int payersCount = 0;

        // Используем цикл for по индексу вместо foreach
        for (int i = 0; i < employees.Count; i++)
        {
            // Проверяем, является ли сотрудник получателем зарплаты
            if (employees[i] is ISalariedEmployee worker)
            {
                sum += worker.TotalSalary();
                payersCount++;
            }
        }

        // Если сотрудников с зарплатой нет, возвращаем 0, иначе считаем среднее
        if (payersCount == 0) 
        {
            return 0;
        }

        return sum / payersCount;
    }
